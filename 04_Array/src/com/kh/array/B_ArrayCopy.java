...
public class B_ArrayCopy {
	...
	public void method1() {
		...
		// 복사본 배열을 수정했는데 원본도 같이 수정됨
		// 이유는 얕은 복사이기때문> 배열의 주소값이 복사되어
	...
	public void method2() {
		// 1. for문 방법(수작업)
		// 새로운 배열을 생성한 후 각 인덱스별로 내부 값을 일일ㅇ히 대입하는 방법
		int[] origin = {1,2,3,4,5};
	...
	// 자바에서 제공하고있는 다양한 메서드 사용해서 배열을 복사
	public void method3() {
		// 2. 새로운 배열을 생성한 후 System 클래스 에서 arraycopy메소드 이용
		
		int [] origin = {1,2,3,4,5};
		int [] copy = new int[10];
		//Sysem.arraycopy(원본 배열명, 원본 배열의 복사를 시작할 인덱스,복사본 배열명, 복사본 배열의 복사될 시작 인덱스, 복사할 개수)
		...
		System.arraycopy(origin, 2, copy, 9, 2); // 0 0 0 0 0 0 0 0 3 4 // 인덱스 범위를 벗어난 복사는 불가능함.
		for(int i =0; i < copy.length; i++) {
			System.out.print(copy[i]+" ");
		}
	}
	public void method4() {
		// 3. Arrays클래스에서 제공하는 copyOf 메소드를 이용
		int [] origin = {1,2,3,4,5};
		...
		/*
		 * 2. System.arraycopy 메서드 : 몇번 인덱스부터 몇개를 어느 위치의 인덱스에 복사할 것인지 세밀하게 지정가능
		 * 3. Arrays.copyOf 메소드 : 무조건 원본 배열의 0번 인덱스부터 내가 제시한 개수만큼 복사 진행됨
	...
	public void method5() {
		...
		/*
		 * Arrays.toString(배열)
		 * -> 앞과 뒤에 [] 각 값마다 ,를 넣어서 하나의 문자열로 연이어서 출력해주는 메소드
		 */
