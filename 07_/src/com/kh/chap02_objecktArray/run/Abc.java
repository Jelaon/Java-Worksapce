...
public class Abc {
	public static void main(String[] args) {
		 ...
		 *
		 *for(순차적으로 접근할 값을 담아줄 변수선언: 순차적으로 접근할 배열/컬렉션){
		 *실행할코드
		 *}
		 *
		 */
		...
		int [] arr2 = {40,50,60};
		for(int num : arr2) { // arr2라는 배열의 0부터 순차적으로 접근하면서 int num이라는 변수에 arr2의 
			// 각 인덱스의 값을 저장함.
		...
		// xx핸드폰의 가격은 xxxx원 입니다. 출력
		// 구매하고자하는 핸드폰이 없다면 
		// 해당핸드폰이 없습니다. 출력
		
		Scanner sc= new Scanner(System.in);
		System.out.print("구매하고자 하는 폰의 이름: ");
		String a = sc.nextLine();
		int b = 0;
		for(Phone phone : phones) {
			if (phone.getName().equals(a)) {
				System.out.printf("%s 핸드폰의 가격은 %d원 입니다.", phone.getName(), phone.getPrice() );
				b = 1;
				break;
			}
		}
