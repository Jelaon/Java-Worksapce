...
public class Run {
	 ...
	 * 2. 객체란? 사전적 의미로는 현실 세계에 독립적으로 존재하는 모든 것을 의미
	 * 3. 객체지향 프로그래밍 : 현실 세계의 객체간의 상호작용(행위)를 프로그래밍을 통해 가상 세계(코드)로 구현하는 과정
	 ...
	 * 5. 추상화 과정
	 *  1) 내가 구현하고자 하는 프로그램에서 필요한 객체들을 생각해볼것 ex)학생 관리 프로그램 ->'학생'객체: 홍길동학생, 민경민학생
	 ...
	 *  	2) 공통적인 속성 및 기능들(이름, 나이, 성별, 주소, 전화번호, 키, 달리기속도, 학년, 반, 국어점수, 영어점수, 등등)
	 *  	3) 만약 학생 '인적사항'을 관리하는 프로그램이라면 ? 이름, 나이, 성별, 주소, 전화번호, 학년, 반 만 추려낸다
	 *  	   만약 학생 '성적'을 관리하는 프로그램이라면 ? 이름, 학년, 반, 국어점수, 영어점수 만 추려낸다
	 ...
	 *  	홍길롱 이라는 학생객체를 만들기 위해선 > String name1 ="홍길동" int age1 = 20; double height = 160.;
	 *  	민경민 이라는 학생객체를 만들기 위해선 > String name2 ="민경민" int age2 = 30; double height = 170.;
	 *  								 > String name3 = ...;
	 *   => 관리하고자 하는 학생들이 늘어나면 늘어날수록 생성해야하는 개수도 엄청나게 증가하게 됨. 즉 관리가 굉장히 힘듦.
	 *   
	 *    	2)"배열"을 이용해서 프로그래밍을 하게 된다면?
	 *    	배열: 하나의 자료형에 여러 개의 값들을 보관할 수 있음.(단, 동일한 자료형일시)
	 ...
	 *    	> 변수를 사용하는 것보단 나아보이지만
	 *     > 하지만 관리하는 학생 중 한명이 전학을 가게되는 경우 해당 인덱스에 있는 모든 키, 나이에 대한 정보도 제거해줘야함
	 *     	그리고 새로운 학생이 들어오게 되는 경우 배열의 특성상 한번 크기를 지정하게 되면 변경하는게 불가능하므로, 
	 *     새로운 정보를 입력하기위해 배열을 복사해서 새롭게 추가해야함. 즉 유지보수가 힘듦.
	 *     
	 *     3)"클래스(구조체)"를 가지고 프로그래밍한다면
	 *     구조체: 한번에 여러개의 자료형을 보관할 수 있는 배열같은 형태.
	 *     String 값도 보관, int값도 보관, double값도 보관할 수 있는 나만의 자료형을 만들 수 있음.
	 *     클래스: 각 개체들의 속성 정보들을 담아내는 그릇과도 같은 존재. 보통 vo패키지 안에 보관한다.
	 *     
	 *     public void Student{
	 *     		String name = "민경민";
	 *     		int age = 30;
	...
	public static void main(String[] args) {
		...
		hong.name = "홍길동";
		hong.age = 20;
		hong.height = 166.7; // 필드 접근하고자한다면 .을 통해 가져올 수 있음( 접근제한자 public인 경우)
		...
		System.out.printf(hong.name+"님의 나이는 "+hong.age+"살이고, 키는 "+ hong.height+"cm입니다. \n");
		System.out.printf("%s님의 나이는 %d살이고, 키는 %.1fcm입니다.", hong.name, hong.age, hong.height);
